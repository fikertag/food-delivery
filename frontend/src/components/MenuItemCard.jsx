...
const MenuItemCard = (catagory, handleClick) => {
  ...
  return (
    <div>
      {items?.length > 0 &&
        items
          .filter((m) => m.itemCatagory == catagory._id)
          .map((item) => (
            <div
              key={item._id}
              className="  flex bg-gray-200 p-4 rounded-lg text-center flex-col align-text-top hover:bg-white transition-all  hover:shadow-black/25 hover:shadow-md"
            >
